...
partCheckboxes.forEach((checkbox) => {
  checkbox.addEventListener('change', function () {
    ...
    const partDescription = partItem.querySelector(
      'span:nth-of-type(2)'
    ).textContent
    const partPriceText = partItem.querySelector(
      'span:nth-of-type(3)'
    ).textContent
    ...
    if (this.checked) {
      ...
      const rowToRemove = partRows.find(
        (row) => row.partNumber === partNumber && row.type === 'flr'
      )
      if (rowToRemove) {
...
function openTMModal(existingRow = null) {
  ...
  if (existingRow) {
    ...
    descriptionInput.value =
      existingRow.querySelector('.part-description').textContent
    ...
    const tmNumber = `TM${Math.floor(Math.random() * 10000)
      .toString()
      .padStart(4, '0')}`
...
function updateTMRow(uniqueId, partNumber, partDescription, newUnitPrice) {
  ...
  const ivRow = selectedPartsList.querySelector(
...
function updateRowDisplay(row, data) {
  if (row) {
    ...
    row.querySelector('.part-price').textContent = `$${(
      data.partPrice * data.partQuantity
...
function createPartRow(
  ...
  partRow.innerHTML = `
    ...
    <div class="selected-part-item-two">
      ...
      <span class="part-price">$${(partPrice * partQuantity).toLocaleString(
        undefined,
        {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
...
function addRowEventListeners(partRow, partPrice, rowType) {
  ...
  partRow
    .querySelector('.minus')
    .parentNode.replaceChild(newMinusButton, partRow.querySelector('.minus'))
  const newPlusButton = partRow.querySelector('.plus').cloneNode(true)
  partRow
    .querySelector('.plus')
    .parentNode.replaceChild(newPlusButton, partRow.querySelector('.plus'))
  ...
  // Quantity input
  oldInputBox.addEventListener('input', () =>
    updateQuantity(0, parseInt(oldInputBox.value) || 1)
  )
  ...
  if (partType !== 'tm') {
    partRow
      .querySelector('.part-description')
      .addEventListener('click', function () {
  ...
  function updateQuantity(change, newQuantity = null) {
    let quantity =
      newQuantity !== null
        ? newQuantity
        : (parseInt(oldInputBox.value) || 1) + change
    ...
    const correspondingRow = otherRow.querySelector(
      `[data-unique-id="${uniqueId}"]`
    )
    if (correspondingRow) {
      correspondingRow.querySelector('.part-quantity').value = quantity
      correspondingRow.querySelector(
        '.part-price'
      ).textContent = `$${newPartPrice.toLocaleString(undefined, {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      })}`
    }
...
function removePartRow(uniqueId, partType) {
  const rowIndex = partRows.findIndex(
    (item) => item.id === uniqueId && item.type === partType
  )
  if (rowIndex > -1) {
...
function updateTotalPrice() {
  totalPriceDisplay.textContent = `Total: $${totalPrice.toLocaleString(
    undefined,
    {
      minimumFractionDigits: 2,
      maximumFractionDigits: 2,
